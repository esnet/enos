...
def setuser(username,password):
	...
	if(username != None and password != None):
		...
		userresource = Resource(username)
		...
		container.deleteResource(username)
...
def removeuser(username):
	...
	if(username != None ):
		...
		raise TypeError("Username cannot be None Type")
...
def listusers():
	...
	query[key] = USERRESOURCE
	users = container.loadResources(query)
	userlist=[]
	for user in users:
		userlist.append(user.getResourceName())
...
def removehypervisor(hypervisor):
	...
	if(hypervisor != None ):
		...
		if(primary and primary.properties[PROPERTIES_HYPERVISOR] == hypervisor):
			...
			if(container.loadResource(hypervisor)):
				container.deleteResource(hypervisor)
